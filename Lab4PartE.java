...
{   
  public static void main(String [] args) throws InterruptedException // you can ignore this throws InterruptedException thing for now
  {
    ...
    Turtle ed = new Turtle(w.getWidth()/2, w.getHeight()/2, w); 
    ...
    int pacmansLastHeading = (int)pacman.getHeading();
    ...
    for (int i = 0; i< 50; i++) {
      ...
      // check if ghost has accidentally backed up into pacman, if so, reverse
      if ((ghost.getXPos() == pacman.getXPos()) && (ghost.getYPos() == pacman.getYPos())) {
        // ghost moved to where pacman is
        ghost.turn(180);
        ghost.forward(100);
      }
      ...
      ed.setHeading(pacmansLastHeading);
      ...
      pacmansLastHeading = (int)pacman.getHeading();
